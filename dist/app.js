...
function render(){
  ...
  const filtered = listings.filter(item => {
    ...
    const text = [item.name,item.former,item.category,item.keywords,item.address,item.phone].join(' ').toLowerCase();
  ...
  summary.textContent = `${filtered.length}件を表示中（初期確認データ）`;
}
